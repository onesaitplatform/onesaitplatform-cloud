...
@Service
@Slf4j
public class MindsDBPredictorMariaDB extends MindsDBPredictorManager {
	...
	public void setUpConnections() {
		...
		predictors.forEach(p -> {
			...
			if (ov != null && ov.getDatasourceId().getConnectionString().contains("mysql")) {
	...
	public String createPredictor(PredictorDTO predictor) {
		...
		ds.setSourceType("mysql_" + ov.getDatasourceId().getId().substring(0, 7));
		final Source s = new Source();
		s.setQuery("SELECT " + predictor.getInputFields() + " FROM " + ov.getDatasourceTableName());
	...
	public boolean supports(Ontology ontology) {
		if (ontology != null && ontology.getRtdbDatasource().equals(RtdbDatasource.VIRTUAL)) {
			...
			return ov.getDatasourceId().getConnectionString().contains("mysql");
		}
		return false;
	}
	...
	public void createDatasource(OntologyVirtualDatasource ovds) {
		...
		mindsDBDatasourcesManager.createDatasource("mysql_" + ovds.getId().substring(0, 7), "mariadb", host, port,
				ovds.getUserId(), JasyptConfig.getEncryptor().decrypt(ovds.getCredentials()));
	}

	@Override
	public boolean supportsByConnName(String connection) {
		return connection != null && connection.startsWith("mysql");
	}
