...
public class GadgetManagementController {
	...
	public ResponseEntity<?> createGadget(
			@ApiParam(value = "GadgetDTOCreate") @RequestBody GadgetDTOCreate gadgetDTO) {
		
		if (gadgetDTO.getIdentification() == null || gadgetDTO.getIdentification().isEmpty() || gadgetDTO.getConfig() == null 
				|| gadgetDTO.getType() == null || gadgetDTO.getDatasource() == null || gadgetDTO.getDescription() == null)
			return new ResponseEntity<>("Missing required fields. Required = [identification, description, datasource, config, type]", HttpStatus.BAD_REQUEST);
		
		if (gadgetService.getGadgetByIdentification(utils.getUserId(), gadgetDTO.getIdentification()) != null) {
			return new ResponseEntity<>("The gadget with identification " + gadgetDTO.getIdentification() +" already exists", HttpStatus.BAD_REQUEST);
		}
	...
	public ResponseEntity<?> updateGadget(
		...
		if (!gadgetService.hasUserPermission(gadget.getId(), user)){
			String eMessage = String.format("The user is not authorized to update the gadget. Identification = %s, userId = %s", gadgetDTO.getIdentification(), user);
			return new ResponseEntity<>(eMessage, HttpStatus.UNAUTHORIZED);
		}
	...
	@PreAuthorize("hasAnyRole('ROLE_ADMINISTRATOR','ROLE_DATASCIENTIST','ROLE_DEVELOPER')")
	public ResponseEntity<?> deleteGadget(@ApiParam(value = "identification") @PathVariable("identification") String identification){
		...
		if (!gadgetService.hasUserPermission(gadget.getId(), user)){
			String eMessage = String.format("The user is not authorized to delete the gadget. Identification = %s, userId = %s",identification, user);
			return new ResponseEntity<>(eMessage, HttpStatus.UNAUTHORIZED);
		}
	...
	private GadgetDatasource retrieveOrCreateDatasource(GadgetDatasourceDTO dto, String gadgetIdentification) {
		...
		Ontology ontology = ontologyService.getOntologyByIdentification(datasourceService.getOntologyFromDatasource(query));
	...
	private GadgetDatasource createDatasource(GadgetDatasourceDTO dto, String gadgetIdentification, Ontology ontology) {
		...
		if (!dto.getIdentification().matches(AppWebUtils.IDENTIFICATION_PATERN)) {
		    throw new GadgetDatasourceServiceException("Identification Datasource Error: Use alphanumeric characters and '-', '_'");
		}
	...
	private GadgetDTO mapGadgetToGadgetDTO(Gadget gadget, GadgetDatasource datasource, List<GadgetMeasure> gadgetMeasures) {
