var KafkaTopicCreateController = function() {
	...
	var handleValidation = function() {
		...
		var form1 = $('#kafka_topic_create_form');
		var error1 = $('.alert-danger');
		var minPartitionValue = kafkaTopicReg.minPartitions;
		// set current language
		currentLanguage = kafkaTopicReg.language || LANGUAGE;
		...
		form1.validate({
					...
					invalidHandler : function(event, validator) { // display error
						...
						toastr.error(kafkaTopicReg.messages.validationKO);
					...
					submitHandler : function(form) {
						console.log("DELETEMEEEEEEE");
						validateConnectionProperties();
						toastr.success(kafkaTopicReg.messages.validationOK);
						form.submit();
					}
				});
	}
				...
				$('#datamodel_properties > tbody')
						.append(
								'<tr data-property="'
										...
										+ '</td><td >'
										+ propertyValue
										+ '</td><td class="icon" style="white-space: nowrap"><span class="btn btn-xs btn-no-border icon-on-table  tooltips color-red" data-container="body" data-placement="bottom" th:title="#{gen.deleteBtn}" th:data-original-title="#{gen.deleteBtn}" onclick="KafkaTopicCreateController.removePropertyRow(this)" th:text="#{device.ontology.remove}"><i class="icon-delete"></i></span></td></tr>');
			}
	...
	var addPropertyRow = function() {
		...
		if (propertySelected === "") {
			toastr.error(messagesForms.operations.genOpError,kafkaTopicReg.propertyNotSelected);
			return false;
		}
		...
		$('#datamodel_properties > tbody')
				.append(
						'<tr data-property="'
								...
								+ '</td><td >'
								+ valueSelected
								+ '</td><td class="icon" style="white-space: nowrap"><span class="btn btn-xs btn-no-border icon-on-table  tooltips color-red" data-container="body" data-placement="bottom" th:title="#{gen.deleteBtn}" th:data-original-title="#{gen.deleteBtn}" onclick="KafkaTopicCreateController.removePropertyRow(this)"><i class="icon-delete"></i></span></td></tr>');
		} else {
			//update record
		...
		$("#propertyValue").val("");
		showHideImageTableProperties();
		toastr.success(messagesForms.operations.genOpSuccess,kafkaTopicReg.messages.propertyAdded);
	}
	 ...
	 var removePropertyRow = function(row) {
		...
		showHideImageTableProperties();
		toastr.success(messagesForms.operations.genOpSuccess,kafkaTopicReg.messages.propertyRemoved);
	...
	var getPropertyDefaultValue = function(prop) {
  	   var properties = kafkaTopicReg.propertiesCatalog;
  	   var defaultValue="";
	...
	var initTemplateElements = function() {
		...
		$("#topic").inputmask({ regex: "[a-zA-Z0-9_-]*", greedy: false });
		
		// set current language and formats
		currentLanguage = kafkaTopicReg.language || LANGUAGE[0];
		...
		$('#resetBtn').on('click', function() {
			cleanFields('kafka_topic_create_form');
		});
		...
		if (kafkaTopicReg.actionMode === null || kafkaTopicReg.actionMode == "") {
		...
		// EDIT MODE ACTION
			...
			$('#replicasNum').prop('readonly', true);
	...
	var showConfirmDeleteDialog = function(id) {
		...
		var connectionId = kafkaTopicReg.kafkaClusterConnectionId;
			...
			buttons : {
				...
				remove : {
					...
					action : function() {
						console.log(id);
						$.ajax({
							url : '/controlpanel/kafka/cluster/' + connectionId + '/topic/' + id,
							headers: {
...
jQuery(document).ready(function() {
	...
	KafkaTopicCreateController.load(kafkaTopicReg);

	// AUTO INIT CONTROLLER.
	KafkaTopicCreateController.init();
});