var AdminGraphController = function() {

	// DEFAULT PARAMETERS, VAR, CONSTS.
	var APPNAME = 'Onesait Platform Control Panel', LIB_TITLE = 'Admin Graph Controller', logControl = 0;
	...
	var drawGraphInfo = function(currentNode) {
		...
		$("#id_nombre").html(currentNode.group);
		$("#id_source").html(currentNode.total);
	...
	var getDataGraph = function() {
		...
		var createSvg = function(title,font,group,label){
			var svg = '';
			...
			svg += '<svg xmlns="http://www.w3.org/2000/svg" width="'+ calculateTextWidth(width,font) +'" height="64">'+
				...
				'<foreignObject x="16" y="16" width="100%" height="100%">'+
					'<div xmlns="http://www.w3.org/1999/xhtml">'+
						groupIcon[group] +
						'<span style="position: absolute; font-family: Soho,Arial; font-size:12px; color:#1B6EAA; top:2px;  left: 20px;font-weight: bolder">'+ title +'</span>'+
						'<span style="position: absolute; font-family: Soho,Arial; font-size:10px; color:#000; top:22px; left: 2px; ">'+ label +'</span>'+
					'</div>'+
			    '</foreignObject>'+
			'</svg>';			
				...
				if ($.inArray(Node.nameTarget, avoidDuplicates) === -1) {
					...
					if (options.groups[group] !== undefined) {
						...
						// ADDING IMAGE GROUP.						
							var font = "12px Soho"; 
							// create svg
							let auxTitle = dataJson.total;
							if(auxTitle == null)
								auxTitle = dataJson.title;
							var svg = createSvg(auxTitle,font,dataJson.group,dataJson.label);							
							// encode as image
	...
	var loadNetwork = function(all) {
		...
		destroyNetwork();
		var url = "/controlpanel/getadmingraph?all="+all;
...
jQuery(document).ready(function() {	
	...
	AdminGraphController.load(graphJson);	
	// AUTO INIT CONTROLLER.
	AdminGraphController.init(false);
});
