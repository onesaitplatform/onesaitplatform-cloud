...
Microservice.Create = (function(){
	...
	var mountableModel = exists.microservice ? '' : $('#table_parameters').find('tr.parameters-model')[0].outerHTML;
	...
	var init = function() {
		...
		$('#btn-microservice-upload').off().on('click',function(){
			$('#btn-microservice-file').click();
		})
...
//		var formAux = $('#form-microservice');
//		formAux.attr("action", "create?" + csrfParameter + "=" + csrfValue)
//		formAux.submit();
		
		$form[0].submit();
	}
	
	function fillParamsIfExist() {
		if(microserviceCreateJson.msName != null)
			document.getElementById('identification').value = microserviceCreateJson.msName[0]
		if(microserviceCreateJson.msPort != null)
			document.getElementById('port').value = microserviceCreateJson.msPort[0]
		if(microserviceCreateJson.msOntologyId != null)
			document.getElementById('ontologies').value = microserviceCreateJson.msOntologyId[0]
		if(microserviceCreateJson.msNotebookId != null)
			document.getElementById('notebooks').value = microserviceCreateJson.msNotebookId[0]
		if(microserviceCreateJson.msContextPath != null) {
			var cp = document.getElementById('context-path').value = microserviceCreateJson.msContextPath[0]
			if (!cp.startsWith("/"))
				cp = "/" + cp;
			document.getElementById('context-path').value = cp;
		}
		if(microserviceCreateJson.msTemplate != null)
			document.getElementById('template').value = microserviceCreateJson.msTemplate[0]
