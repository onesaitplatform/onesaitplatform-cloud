...
grep -rl '${HTTP_PROXY}' /opt/nodeRed/Flow-Engine-Manager | xargs sed -i 's/${HTTP_PROXY}/'"$HTTP_PROXY"'/g'
grep -rl '${HTTPS_PROXY}' /opt/nodeRed/Flow-Engine-Manager | xargs sed -i 's/${HTTPS_PROXY}/'"$HTTPS_PROXY"'/g'
grep -rl '${NO_PROXY}' /opt/nodeRed/Flow-Engine-Manager | xargs sed -i 's/${NO_PROXY}/'"$NO_PROXY"'/g'
...
grep -rl '${COMMENT_PROXY_START}' /opt/nodeRed/Flow-Engine-Manager | xargs sed -i 's/${COMMENT_PROXY_START}/'"$COMMENT_PROXY_START"'/g'
grep -rl '${COMMENT_PROXY_END}' /opt/nodeRed/Flow-Engine-Manager | xargs sed -i 's/${COMMENT_PROXY_END}/'"$COMMENT_PROXY_END"'/g'
