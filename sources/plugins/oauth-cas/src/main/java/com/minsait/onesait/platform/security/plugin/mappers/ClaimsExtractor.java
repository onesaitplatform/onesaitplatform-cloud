...
public class ClaimsExtractor {
	...
	private static final String MAIL_SUFFIX = "@";
	@Value("${onesaitplatform.authentication.cas.attributes.mail_suffix:onesaitplatform.com:laspalmasgc.es}")
	...
	public User mapFromClaims(Map<String, Object> map) throws JsonProcessingException {
		...
		user.setFullName(claims.getGivenName());
		user.setUsername((String) principalExtractor.extractPrincipal(map));
		user.setMail(StringUtils.isEmpty(claims.getEmail()) ? claims.getPrincipal() + MAIL_SUFFIX + mailSuffix
				: claims.getEmail());
