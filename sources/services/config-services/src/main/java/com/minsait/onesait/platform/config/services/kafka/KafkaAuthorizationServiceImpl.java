...
public class KafkaAuthorizationServiceImpl implements KafkaAuthorizationService {
	...
	public void addAclToOntologyClient(ClientPlatformOntology clientPlatformOntology) {
		...
		if (kafkaService != null) {
			if (clientPlatformOntology.getOntology().isAllowsCreateTopic()) {
				kafkaService.addAcls(clientPlatformOntology.getClientPlatform().getIdentification(),
						clientPlatformOntology.getAccess().toString(), clientPlatformOntology.getOntology().getTopic());
			}
			if (clientPlatformOntology.getOntology().isAllowsCreateNotificationTopic()) {
				kafkaService.addAcls(clientPlatformOntology.getClientPlatform().getIdentification(),
						clientPlatformOntology.getAccess().toString(),
						clientPlatformOntology.getOntology().getNotificationTopic());
			}
		} else {
			log.warn("Kafka is not enabled, skipping ACL creation.");
		}
	...
	public void removeAclToOntologyClient(ClientPlatformOntology clientPlatformOntology) {
		...
		if (kafkaService != null) {
			if (clientPlatformOntology.getOntology().isAllowsCreateTopic()) {
				kafkaService.deleteAcls(clientPlatformOntology.getClientPlatform().getIdentification(),
						clientPlatformOntology.getOntology().getTopic());
			}
			if (clientPlatformOntology.getOntology().isAllowsCreateNotificationTopic()) {
				kafkaService.deleteAcls(clientPlatformOntology.getClientPlatform().getIdentification(),
						clientPlatformOntology.getOntology().getNotificationTopic());
			...
			log.warn("Kafka is not enabled, skipping ACL removal.");
		}
	}
