...
public class PrometheusServiceImpl implements PrometheusService {
    ...
    private String invokePrometheusAPI(String prometheusQuery) {
        ...
        UriComponentsBuilder builder = UriComponentsBuilder.fromUriString(resourcesService.getUrl(Module.PROMETHEUS, ServiceUrl.BASE)).queryParam("query", prometheusQuery);
    ...
    public String getMemStats(String namespace) {
        ...
        String memoryUsedString = df.format(memoryUsed / 1000000000) + " of " + df.format(totalMem / 1000000000) + "GB Used";
	...
	private ArrayList<PodStat> toPodMemUseStatArray(JSONArray usageResults) {
    	...
    	for (int i = 0; i < usageResults.length(); i++) {
            ...
            if (result.getJSONObject("metric").has("container_name") && !result.getJSONObject("metric").getString("container_name").equals("POD")) {
	...
	private JSONArray sumPodMemUsageValues(ArrayList<PodStat> podMemStatArray) {
					...
					if (podStatMemsum.getPod().equals(podStatIt.getPod())) {
						for (Long usageTime : podStatIt.getPodUse().keySet()) {
							if (podStatMemsum.getPodUse().get(usageTime)!=null) {
								podStatMemsum.getPodUse().put(usageTime, podStatMemsum.getPodUse().get(usageTime) + podStatIt.getPodUse().get(usageTime));
							} else {
        ...
        for (PodStat podStat : podStatMap.values()) {
    		...
    		List<Entry<Long, Long>> sumPodValuesList = podStat.getPodUse().entrySet().stream().collect(Collectors.toList());
        	...
        	sumPodTotalUsageValue.put(new Double(sumPodValuesList.get(sumPodValuesList.size() - 1).getValue()) / 1000000000);
	...
	private JSONArray createCPUIncrementalUse(List<Entry<Long, Long>> sumvaluesList, double numCpusValue) {
		...
		for (int i = 1; i < sumvaluesList.size(); i++) {
            JSONArray valuearray = new JSONArray();
            valuearray.put(new Double(sumvaluesList.get(i).getKey() / 1000));
            valuearray.put(new Double(((sumvaluesList.get(i).getValue() - sumvaluesList.get(i - 1).getValue())) * 100 / 60) / numCpusValue);
	...
	private JSONArray sumPodCPUUsageValues(ArrayList<PodStat> podCPUStatArray, double numCpusValue) {
					...
					if (podStatCPUsum.getPod().equals(podStatIt.getPod())) {
						for (Long usageTime : podStatIt.getPodUse().keySet()) {
							if (podStatCPUsum.getPodUse().get(usageTime)!=null) {
								podStatCPUsum.getPodUse().put(usageTime, podStatCPUsum.getPodUse().get(usageTime) + podStatIt.getPodUse().get(usageTime));
							} else {
        ...
        for (PodStat podStat : podStatMap.values()) {
        	...
        	JSONArray sumPodTotalUsageValue = new JSONArray();
        	sumPodTotalUsageValue.put(new Double(sumPodValuesList.get(sumPodValuesList.size() - 1).getKey()) / 1000);
        	sumPodTotalUsageValue.put(new Double(((sumPodValuesList.get(sumPodValuesList.size() - 1).getValue() - sumPodValuesList.get(sumPodValuesList.size() - 2).getValue())) * 100 / 60 ) / numCpusValue);
        	sumPodUsageValue.put(sumPodTotalUsageValue);
