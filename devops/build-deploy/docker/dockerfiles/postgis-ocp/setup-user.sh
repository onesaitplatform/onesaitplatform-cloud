...
RESULT=`psql postgres -t -c "SELECT 1 FROM pg_roles WHERE rolname = '$POSTGRES_USER'"`
COMMAND="ALTER"
if [ -z "$RESULT" ]; then
        COMMAND="CREATE"
        createdb -U $(whoami) $(whoami)
fi
psql postgres -c "$COMMAND USER $POSTGRES_USER WITH SUPERUSER ENCRYPTED PASSWORD '$POSTGRES_PASS';"

replication_password
REPLICATION_PASS=$(cat /tmp/REPLPASSWORD.txt)

echo "Creating replication user $REPLICATION_USER"
RESULT_REPLICATION=`psql postgres -t -c "SELECT 1 FROM pg_roles WHERE rolname = '$REPLICATION_USER'"`
COMMANDS="ALTER"
if [ -z "$RESULT_REPLICATION" ]; then
  COMMANDS="CREATE"
fi
psql postgres -c "$COMMANDS USER $REPLICATION_USER WITH REPLICATION ENCRYPTED PASSWORD '$REPLICATION_PASS';"
