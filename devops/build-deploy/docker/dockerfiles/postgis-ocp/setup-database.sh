...
if [[ -z "$(ls -A ${DATADIR} 2> /dev/null)" || "${RECREATE_DATADIR}" =~ [Tt][Rr][Uu][Ee] ]]; then
    ...
    command="$INITDB -U $(whoami) --pwfile=/scripts/pwfile -E ${DEFAULT_ENCODING} --lc-collate=${DEFAULT_COLLATION} --lc-ctype=${DEFAULT_CTYPE} --wal-segsize=${WAL_SEGSIZE} --auth=${PASSWORD_AUTHENTICATION} -D ${DATADIR} ${INITDB_WALDIR_FLAG} ${INITDB_EXTRA_ARG}"
    echo "$command"
    $command
    chmod -R 700 ${DATADIR}
else
...
${POSTGRES} -D ${DATADIR} -c config_file=${CONF} ${LOCALONLY} &
...
if [[ "$(boolean ${POSTGRES_TEMPLATE_EXTENSIONS})" == TRUE ]] ; then
    for ext in $(echo ${POSTGRES_MULTIPLE_EXTENSIONS} | tr ',' ' '); do
        ...
        psql -c "CREATE EXTENSION IF NOT EXISTS \"${ext}\" cascade;" template1
    done
fi
...
for db in $(echo ${POSTGRES_DBNAME} | tr ',' ' '); do
        RESULT=`psql -t -c "SELECT count(1) from pg_database where datname='${db}';"`
        ...
        if [[  ${RESULT} -eq 0 ]]; then
            ...
            for ext in $(echo ${POSTGRES_MULTIPLE_EXTENSIONS} | tr ',' ' '); do
                ...
                if [[ ${ext} = 'pg_cron' ]]; then
                  ...
                  psql -c "CREATE EXTENSION IF NOT EXISTS \"${ext}\" cascade;" $db
                fi
            done
            echo "Loading legacy sql"
            psql ${db} -f ${SQLDIR}/legacy_minimal.sql || true
            psql ${db} -f ${SQLDIR}/legacy_gist.sql || true
...
if [ ! -f "${CRON_LOCKFILE}" ]; then
        psql -c 'CREATE EXTENSION IF NOT EXISTS pg_cron cascade;' ${SINGLE_DB}
        touch ${CRON_LOCKFILE}
fi
