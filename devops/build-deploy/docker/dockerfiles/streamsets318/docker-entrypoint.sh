...
then
    ...
    if [ -f "${SDC_CONF}/sdc-log4j-graylog.properties" ]; then
    echo "${SDC_CONF}/sdc-log4j-graylog.properties exists."
    mv ${SDC_CONF}/sdc-log4j-graylog.properties ${SDC_CONF}/sdc-log4j.properties
    grep -rl '${GRAYLOG_HOST}' ${SDC_CONF}/sdc-log4j.properties | xargs sed -i 's/${GRAYLOG_HOST}/'"$GRAYLOG_HOST"'/g'
    grep -rl '${GRAYLOG_PORT}' ${SDC_CONF}/sdc-log4j.properties | xargs sed -i 's/${GRAYLOG_PORT}/'"$GRAYLOG_PORT"'/g'
    grep -rl '${GRAYLOG_APP_NAME}' ${SDC_CONF}/sdc-log4j.properties | xargs sed -i 's/${GRAYLOG_APP_NAME}/'"$GRAYLOG_APP_NAME"'/g'
    fi
else
    if [ -f "/etc/sdc/sdc-log4j-graylog.properties" ]; then
		rm /etc/sdc/sdc-log4j-graylog.properties
	fi
fi

cp -rf /tmp/streamsets-libs /opt/streamsets-datacollector-3.18.1
