...
export default class SelectComponent extends ListComponent {
  ...
  getCustomItems() {
    ...
    if (this.component.dataSrc === 'custom') {
      customItems = this.evaluate(this.component.data.custom, {
        values: []
      }, 'values');
    }
    else {
      if ( this.component.filterDatasource && this.component.filterDatasource.length > 0 && this.component.refreshOn ) {
        var filters = [];
        var valueFilter = this.getDataFromPath( this.parent.submission.data,this.component.refreshOn );
        if ( !valueFilter || valueFilter.length === 0 ) {
          customItems = this.evaluate(`from('${this.component.datasource}').exec().then(function( msg ) {instance.setItems(msg); });`, {
            values: []
          }, 'values');
        }
        else if (valueFilter && !Array.isArray(valueFilter)) {
          if (typeof valueFilter === 'string') {
            valueFilter = `'${valueFilter}'`;
          }
        filters.push( { field: this.component.filterDatasource, op: '=', exp:  valueFilter } );
          customItems = this.evaluate(`from('${this.component.datasource}').filter(${JSON.stringify(filters)}).exec().then(function( msg ) {instance.setItems(msg); });`, {
          values: []
        }, 'values');
        }
        else if (valueFilter && Array.isArray(valueFilter)) {
          var expres ='';
          if ( typeof valueFilter[0] === 'string') {
            expres = `('${valueFilter.join("', '")}')`;
          }
          else {
            expres = `(${valueFilter.join(',')})`;
          }
        filters.push( { field: this.component.filterDatasource, op: 'IN', exp:expres } );
          customItems = this.evaluate(`from('${this.component.datasource}').filter(${JSON.stringify(filters)}).exec().then(function( msg ) {instance.setItems(msg); });`, {
          values: []
        }, 'values');
      }
  ...
  updateDatasourceItems(forceUpdate) {
    if (this.asyncCustomValues()) {
      if (!forceUpdate && !this.active) {
        this.itemsLoadedResolve();
        return;
  ...
  update() {
    ...
    if (this.component.dataSrc === 'datasource') {
      this.updateDatasourceItems();
    }
  ...
  asString(value) {
    ...
    if (['values', 'custom','datasource'].includes(this.component.dataSrc) && !this.asyncCustomValues()) {
      const {
        items,
        valueProperty,
