#!/bin/bash
set -Eeo pipefail
grep -rl '${LOG_LEVEL}' /etc/ssh/sshd_config | xargs sed -i 's/${LOG_LEVEL}/'"$LOG_LEVEL"'/g'
...
if $startSshd; then
    log "Executing sshd and cron"
    exec /tmp/cron.sh &
    exec /monitor_ssh_logs.sh &
    exec /usr/sbin/sshd -D -f /etc/ssh/sshd_config -E /etc/ssh/ssh_fifo
else
    log "Executing $*"
    exec "$@"
fi
